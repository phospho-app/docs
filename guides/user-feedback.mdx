...
This function is different from the server side, because it doesn't need authentication. However, it requires the `projectId` to be passed as an argument.

After logging a task, you can send it back to the browser and use the `sendUserFeedback` function. 
