...
The phospho logger is designed to use as little resources as possible. The logger runs on a separate, asynchronous thread. All the heavy computation happens asynchronously in the phospho backend. The logger is non-blocking. The logger won't raise any Exception in your app if the phospho's servers are unavailable.