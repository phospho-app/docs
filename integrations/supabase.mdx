...
Then, add those environment variables to your Supabase project.

### Option 1: In the CLI

Add the phospho API key and project id to your `./supabse/.env` file:
...
Push those environment variables to your Supabase project:
...
You can also add the phospho API key and project id directly in the Supabase console. Go to Settings/Environment variables, and create the `PHOSPHO_API_KEY` and `PHOSPHO_PROJECT_ID` variables.

<img width={"100%"} src="../images/supabase/secrets_edge_functions.png" />
...
Let's create a [Supabase Edge Function](https://supabase.com/docs/guides/functions/quickstart) that will log the chat message to phospho using the [phospho API](/api-reference). Later, we will trigger this function with a Supabase Database webhook.
...
<img width={"40%"} src="../images/supabase/webhook_tab.png" />
...
<img width={"80%"} src="../images/supabase/create_webhook_1.png" />
...
<img width={"80%"} src="../images/supabase/create_webhook_2.png" />

### Test the webhook

To test the webhook, send a message to your chatbot. You should see a new row in the `chat_history` table, and the webhook should be triggered.
...
Learn more about phospho features by reading the [guides](/guides):

<CardGroup>
<Card title="Log user feedback" href="/guides/user-feedback">
    Log user feedback to phospho to improve the phospho evaluation
</Card>
<Card title="Run AB Tests" href="/guides/ab-test">
