...
### How to control the SO-100 with a game controller?
...
<Info>If you didn't buy a starter pack, you can still get access to the Meta Quest app by [purchasing it](https://robots.phospho.ai)</Info>
...
<Info>
  **Troubleshooting if you don't see the server:**
    1. Make sure the **port** field in the Meta Quest app settings matches the port of your phosphobot server (default is `80`, then `8020`).
    2. Use the manual connection option by *Adding to favourite* your phosphobot server's IP address (e.g., `192.168.1.X`) and port.
    3. Your computer may have a firewall blocking the connection. Make sure the port is open and check your local network settings.
</Info>
