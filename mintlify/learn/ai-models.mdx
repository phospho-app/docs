...
uv run lerobot/lerobot/scripts/train.py \
 --dataset.repo_id=PLB/simple-lego-pickup-mono-2 \ # Replace with <HF_USERNAME>/<DATASET_NAME>
...
uv pip install -r https://raw.githubusercontent.com/phospho-app/phosphobot/main/inference/ACT/requirements.txt

# Download inference script
curl -o act_server.py https://raw.githubusercontent.com/phospho-app/phosphobot/main/inference/ACT/inference.py

# Start server (adjust --device based on hardware)
uv run act_server.py -- --device=cuda --model_id=<YOUR_HF_MODEL_ID>
```
...
3. Create inference client script (Copy the content into `my_model/inference.py`):
