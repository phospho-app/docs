...
<Warning> 
This Tutorial May Become Outdated
...
2. **Fixing config.json** You need to change `n_action_steps` in the `config.json` file. The default value is set to 1, but for training SmolVLA, it should be set to 50. This is crucial for the model to learn to predict a sequence of actions rather than just a single action step.
...
2.  **Uploading the Model to the Hub:**
...
Here is a draft of a technical step by step guide. Improve it using the instructions for this other video. 
- Add the missing steps
- Fix the uv installation to be the one in the video
- explain that it trains very slowly on macbook pro
- explain that in google colab you need to "Note: If you still want to run on Google Colab, the fix is to downgrade torchcodec to 0.2.1 ( pip install torchcodec==0.2.1)."
- Add any tips and tricks about dependencies in python etc. 
- Make sure to add code snippets and many details for troubleshooting, in order to be as helpful as possible with beginner developers not familiar with uv, machine learning, python, or robotics

Make the tutorial self standing. Do not mention the video. Make it SEO friendly and easy to find on the internet, for example using common questions. 
...
*   **Problem:** The SmolVLA model outputs actions in the same units as its training data. The `PLB/phospho-playground-mono` dataset uses radians. However, the LeRobot SO-100 driver expects actions in degrees. This will cause the robot to move erratically or barely at all.
