...
F=K(xdes−x)+Bx˙F = K(x_{des} - x) + B\dot{x}F=K(xdes​−x)+Bx˙
Where KK
K is stiffness and BB
B is damping.

Our approach inverts this relationship:
xdes=x+K−1Fx_{des} = x + K^{-1}Fxdes​=x+K−1F
In our case, FF
F is the gravity force, and K−1K^{-1}
K−1 is represented by the α\alpha
α parameter.
